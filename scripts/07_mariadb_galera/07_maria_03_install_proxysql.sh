#!/usr/bin/env bash
#
# 07_maria_03_install_proxysql.sh - Installation ProxySQL pour MariaDB Galera
#
# Ce script installe et configure ProxySQL sur 2 nœuds pour équilibrer
# les connexions vers le cluster MariaDB Galera.
#
# Usage:
#   ./07_maria_03_install_proxysql.sh [servers.tsv]
#
# Prérequis:
#   - Script 07_maria_02_deploy_galera.sh exécuté
...
# Header
echo "=============================================================="
echo " [KeyBuzz] Module 7 - Installation ProxySQL pour MariaDB Galera"
echo "=============================================================="
echo ""
...
while IFS=$'\t' read -r ENV IP_PUBLIQUE HOSTNAME IP_PRIVEE FQDN USER_SSH POOL ROLE SUBROLE DOCKER_STACK CORE NOTES <&3; do
    ...
    if [[ "${ROLE}" == "db_proxy" ]] && [[ "${SUBROLE}" == "proxysql" ]]; then
        if [[ -n "${IP_PRIVEE}" ]]; then
            if [[ "${HOSTNAME}" == "proxysql-01" ]] || \
               [[ "${HOSTNAME}" == "proxysql-02" ]]; then
...
if [[ ${#PROXYSQL_NODES[@]} -lt 1 ]]; then
    log_error "Aucun nœud ProxySQL trouvé (attendu: 1-2)"
    exit 1
fi
...
# Déployer ProxySQL sur chaque nœud
...
log_info "Vérification de la connectivité ProxySQL..."
for i in "${!PROXYSQL_NODES[@]}"; do
    hostname="${PROXYSQL_NODES[$i]}"
    ip="${PROXYSQL_IPS[$i]}"
...
echo ""
log_info "Prochaine étape: Configurer le LB Hetzner (10.0.0.20:3306)"
